...
def sendtracking(order, customer_email, trackingnum, customer_rep_email, sender_email, password):
...
def sendlabel(order, label, order_num, shipper_email, sender_email, password): #don's email or vennite
    # Create email
    msg = MIMEMultipart()
    msg["From"] = sender_email
    msg["To"] = shipper_email
    ...
    with smtplib.SMTP("smtp.gmail.com", 587) as server:
        ...
        server.login(sender_email, password)
        server.sendmail(sender_email, shipper_email, msg.as_string())


def send_update(order, customer_email, tracking_num, status, sender_email, password):
